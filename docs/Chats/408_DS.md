...
### 斐波那契数列是满足 F(0)= 1, F(1)= 1, F(n)= F(n-1)+F(n-2)(n≥2)的数列，数列的前几项为 1,1,2,3， 5,8,13,21,…。写出求解斐波那契数列第 n 项的程序。
...
### 在一棵以二叉链表为存储结构的二叉树中，查找 data 域值等于 key 的结点是否存在，如果存在，则将指针 q 指向该结点，假设 data 为 int 型。（二叉树中结点值都不相同）
...
### 假设二叉树采用二叉链表形式存储，设计一个算法，求先序遍历序列中第 k(1≤k≤ 二叉树中结点个数)个结点的值。
...
### 设树 B 是一棵采用二叉链表形式存储的二叉树编写一个把树 B 中所有结点的左、右子树进行交换的函数。
...
### 假设二叉树采用二叉链表存储结构，设计一个算法，求二叉树 T 中值为 x 的结点的层次号。
...
### 假设二叉树采用二叉链表存储结构，设计一个算法，求给定的二叉树宽度（即具有结点数最多的那一层的结点个数)。<span style="font-size:0.6em; text-decoration:line-through;">(有辣么一坨变量,啊啊啊)</span>
...
int Width(BiTree T){
    ...
    // while循环结束,level里存有这样的数据 |1|2|2|3| 对应 |第一层|第二层|第三层
    int max = 0,n,i = 0; // i 是用于遍历level的
    k = 1; // 重置k
    while(i < rear){
        n = 0;
